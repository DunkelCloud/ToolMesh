...
func TestExecuteTool_GateRejects(t *testing.T) {
	...
	exec := New(nil, nil, mb, gate.NewPipeline([]gate.Evaluator{g}), nil, 120*time.Second, logger, nil, nil)
...
func TestCallerCredentialDurchstich(t *testing.T) {
	...
	exec := New(nil, credStore, mb, pipeline, nil, 120*time.Second, newTestLogger(), nil, nil)
...
func TestCallerCredentialDurchstich_PostGateWithCallerClass(t *testing.T) {
	...
	exec := New(nil, nil, mb, gate.NewPipeline([]gate.Evaluator{g}), nil, 120*time.Second, newTestLogger(), nil, nil)
